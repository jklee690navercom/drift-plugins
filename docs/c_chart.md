...
## 개요
일정 단위에서 발생하는 결함/사건의 건수를 포아송 분포 기반 제어 한계로 모니터링하는 제어 차트(control chart) 기반 drift 탐지 알고리즘이다.
...
### 효과적인 경우
- 결함 수, 에러 발생 횟수, 사고 건수 등 카운트 데이터를 모니터링할 때
- 검사 단위의 크기가 일정할 때
- 데이터가 포아송 분포를 따르는 경우
- 구현이 단순하고 해석이 직관적인 알고리즘이 필요할 때

### 한계점
- 포아송 분포 가정이 성립하지 않으면 (과분산/과소분산) 제어 한계가 부정확하다
- 검사 단위의 크기가 변하면 C Chart 대신 U Chart를 사용해야 한다
- 연속형 데이터에는 적합하지 않다 (정수형 카운트 데이터 전용)
- $\bar{c}$가 매우 작으면(예: 0에 가까우면) 제어 한계가 의미 없어질 수 있다
