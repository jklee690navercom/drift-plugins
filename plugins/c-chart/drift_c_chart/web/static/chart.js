...
function renderExpertCharts(data) {
    ...
    const ctx = document.getElementById('c-chart-expert');
    if (!ctx) return;
    if (cChartExpert) cChartExpert.destroy();
    ...
    const datasets = [
        {
            ...
            borderColor: '#6366f1',
            borderWidth: 1.5,
            pointRadius: values.map(v => (v > ucl || v < lcl) ? 4 : 0),
    ...
    cChartExpert = new Chart(ctx.getContext('2d'), {
        type: 'line',
