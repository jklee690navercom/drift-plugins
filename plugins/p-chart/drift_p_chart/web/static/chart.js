...
function renderExpertCharts(data) {
    ...
    const ctx = document.getElementById('p-chart-expert');
    if (!ctx) return;
    if (pChartExpert) pChartExpert.destroy();
    ...
    const datasets = [
        {
            ...
            pointBackgroundColor: values.map(v => (v > ucl || v < lcl) ? '#ef4444' : '#6366f1'),
            fill: false,
        },
        {
    ...
    pChartExpert = new Chart(ctx.getContext('2d'), {
        type: 'line',
        ...
        options: {
            ...
            scales: {
                ...
                y: { display: true, title: { display: true, text: 'Proportion' } }
            },
